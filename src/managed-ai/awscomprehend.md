...
Comprehend has advanced settings such as custom classification. Here we define how we want Comprehend to categorize the documents for ourselves, so we define them.
...
2. We create training data and put it in Amazon S3
3. Feed it into Amazon Comprehend, which builds and trains internally a custom classifier
4. When a document arrives (email or whatever you want), the custom classifier will say "this looks like a complaint document" based on how you've defined what complaints look like
...
**Example:**
In a sample text, named entity recognition can recognize that:
...
We also have the option to make Comprehend recognize custom entities. Here we want to analyze the text for specific terms and noun-based phrases.
...
**How it works:**
1. Train the model with a list of the entities you're looking for and documents that contain them
2. Give examples to Comprehend
3. A custom entity recognizer gets trained
4. Use it to look for policy numbers within your documents

This can be used for real-time or asynchronous analysis.
